!function(t,n){"object"==typeof exports&&"undefined"!=typeof module?n(exports,require("@angular/platform-browser"),require("rxjs"),require("rxjs/operators"),require("@angular/cdk/portal"),require("@angular/common"),require("@angular/core"),require("ng-zorro-antd/core"),require("ng-zorro-antd/i18n")):"function"==typeof define&&define.amd?define("ng-zorro-antd/empty",["exports","@angular/platform-browser","rxjs","rxjs/operators","@angular/cdk/portal","@angular/common","@angular/core","ng-zorro-antd/core","ng-zorro-antd/i18n"],n):n((t["ng-zorro-antd"]=t["ng-zorro-antd"]||{},t["ng-zorro-antd"].empty={}),t.ng.platformBrowser,t.rxjs,t.rxjs.operators,t.ng.cdk.portal,t.ng.common,t.ng.core,t["ng-zorro-antd"].core,t["ng-zorro-antd"].i18n)}(this,function(t,n,I,e,g,o,i,M,r){"use strict";var c=new i.InjectionToken("nz-empty-content"),a=new i.InjectionToken("nz-empty-component-name"),N="data:image/svg+xml;base64,",s="data:image/svg+xml;base64,";var u=function(){function t(t){this.defaultEmptyContent=t,this.userDefaultContent$=new I.BehaviorSubject(undefined),this.defaultEmptyContent&&this.userDefaultContent$.next(this.defaultEmptyContent)}return t.prototype.setDefaultContent=function(t){if(!("string"==typeof t||t===undefined||null===t||t instanceof i.TemplateRef||t instanceof i.Type))
/**
     * @license
     * Copyright Alibaba.com All Rights Reserved.
     *
     * Use of this source code is governed by an MIT-style license that can be
     * found in the LICENSE file at https://github.com/NG-ZORRO/ng-zorro-antd/blob/master/LICENSE
     */
throw function n(t){return TypeError("[NG-ZORRO]: useDefaultContent expect 'string', 'templateRef' or 'component' but get "+t)}(t);this.userDefaultContent$.next(t)},t.prototype.resetDefault=function(){this.userDefaultContent$.next(undefined)},t.decorators=[{type:i.Injectable,args:[{providedIn:"root"}]}],t.ctorParameters=function(){return[{type:i.Type,decorators:[{type:i.Inject,args:[c]},{type:i.Optional}]}]},t.ngInjectableDef=i.defineInjectable({factory:function(){return new t(i.inject(c,8))},token:t,providedIn:"root"}),t}(),D=function(){function t(t,n,e,g,o){this.emptyService=t,this.sanitizer=n,this.viewContainerRef=e,this.cdr=g,this.injector=o,this.contentType="string",this.defaultSvg=this.sanitizer.bypassSecurityTrustResourceUrl(s),this.size="",this.subs_=new I.Subscription}return t.prototype.ngOnChanges=function(t){t.nzComponentName&&(this.size=this.getEmptySize(t.nzComponentName.currentValue)),t.specificContent&&!t.specificContent.isFirstChange()&&(this.content=t.specificContent.currentValue,this.renderEmpty())},t.prototype.ngOnInit=function(){var n=this,t=this.emptyService.userDefaultContent$.subscribe(function(t){n.content=n.specificContent||t,n.renderEmpty()});this.subs_.add(t)},t.prototype.ngOnDestroy=function(){this.subs_.unsubscribe()},t.prototype.getEmptySize=function(t){switch(t){case"table":case"list":return"normal";case"select":case"tree-select":case"cascader":case"transfer":return"small";default:return""}},t.prototype.renderEmpty=function(){var t=this.content;if("string"==typeof t)this.contentType="string";else if(t instanceof i.TemplateRef){var n={$implicit:this.nzComponentName};this.contentType="template",this.contentPortal=new g.TemplatePortal(t,this.viewContainerRef,n)}else if(t instanceof i.Type){n=new WeakMap([[a,this.nzComponentName]]);var e=new g.PortalInjector(this.injector,n);this.contentType="component",this.contentPortal=new g.ComponentPortal(t,this.viewContainerRef,e)}else this.contentType="string",this.contentPortal=undefined;this.cdr.markForCheck()},t.decorators=[{type:i.Component,args:[{changeDetection:i.ChangeDetectionStrategy.OnPush,encapsulation:i.ViewEncapsulation.None,selector:"nz-embed-empty",exportAs:"nzEmbedEmpty",template:'<ng-container *ngIf="!content && specificContent !== null" [ngSwitch]="size">\n  <nz-empty *ngSwitchCase="\'normal\'" class="ant-empty-normal" [nzNotFoundImage]="defaultSvg"></nz-empty>\n  <nz-empty *ngSwitchCase="\'small\'" class="ant-empty-small" [nzNotFoundImage]="defaultSvg"></nz-empty>\n  <nz-empty *ngSwitchDefault></nz-empty>\n</ng-container>\n<ng-container *ngIf="content">\n  <ng-template *ngIf="contentType !== \'string\'" [cdkPortalOutlet]="contentPortal"></ng-template>\n  <ng-container *ngIf="contentType === \'string\'">\n    {{ content }}\n  </ng-container>\n</ng-container>\n'}]}],t.ctorParameters=function(){return[{type:u},{type:n.DomSanitizer},{type:i.ViewContainerRef},{type:i.ChangeDetectorRef},{type:i.Injector}]},t.propDecorators={nzComponentName:[{type:i.Input}],specificContent:[{type:i.Input}]},t}(),C=function(){function t(t,n,e){this.sanitizer=t,this.i18n=n,this.cdr=e,this.defaultSvg=this.sanitizer.bypassSecurityTrustResourceUrl(N),this.isContentString=!1,this.locale={},this.destroy$=new I.Subject}return Object.defineProperty(t.prototype,"shouldRenderContent",{get:function(){var t=this.nzNotFoundContent;return!(!t&&"string"!=typeof t)},enumerable:!0,configurable:!0}),t.prototype.ngOnChanges=function(t){var n=t.nzNotFoundContent;n&&(this.isContentString="string"==typeof n.currentValue)},t.prototype.ngOnInit=function(){var t=this;this.i18n.localeChange.pipe(e.takeUntil(this.destroy$)).subscribe(function(){t.locale=t.i18n.getLocaleData("Empty"),t.cdr.markForCheck()})},t.prototype.ngOnDestroy=function(){this.destroy$.next(),this.destroy$.complete()},t.decorators=[{type:i.Component,args:[{changeDetection:i.ChangeDetectionStrategy.OnPush,encapsulation:i.ViewEncapsulation.None,selector:"nz-empty",exportAs:"nzEmpty",template:'<div class="ant-empty-image">\n  <ng-container *nzStringTemplateOutlet="nzNotFoundImage">\n    <img [src]="nzNotFoundImage || defaultSvg" [alt]="isContentString ? nzNotFoundContent : \'empty\'">\n  </ng-container>\n</div>\n<p class="ant-empty-description">\n  <ng-container *nzStringTemplateOutlet="nzNotFoundContent">\n    {{ shouldRenderContent ? nzNotFoundContent : locale[\'description\'] }}\n  </ng-container>\n</p>\n<div class="ant-empty-footer" *ngIf="nzNotFoundFooter">\n  <ng-container *nzStringTemplateOutlet="nzNotFoundFooter">\n    {{ nzNotFoundFooter }}\n  </ng-container>\n</div>\n',host:{"class":"ant-empty"},styles:["nz-empty { display: block; }"]}]}],t.ctorParameters=function(){return[{type:n.DomSanitizer},{type:r.NzI18nService},{type:i.ChangeDetectorRef}]},t.propDecorators={nzNotFoundImage:[{type:i.Input}],nzNotFoundContent:[{type:i.Input}],nzNotFoundFooter:[{type:i.Input}]},t}(),y=function(){function t(){}return t.decorators=[{type:i.NgModule,args:[{imports:[o.CommonModule,g.PortalModule,M.NzAddOnModule,r.NzI18nModule],declarations:[C,D],exports:[C,D]}]}],t}();t.NzEmbedEmptyComponent=D,t.NzEmptyComponent=C,t.NzEmptyModule=y,t.NzEmptyService=u,t.NZ_DEFAULT_EMPTY_CONTENT=c,t.NZ_EMPTY_COMPONENT_NAME=a,t.emptyImage=N,t.simpleEmptyImage=s,Object.defineProperty(t,"__esModule",{value:!0})});
//# sourceMappingURL=ng-zorro-antd-empty.umd.min.js.map